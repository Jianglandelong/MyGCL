...
class Encoder(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight=None):
        ...
        adj1_lp = self.get_adj(edge_index1)
        adj2_lp = self.get_adj(edge_index2)
    ...
    def get_embedding(self, x, edge_index, edge_weight=None):
        adj_lp = self.get_adj(edge_index)
...
class CrossViewContrast(torch.nn.Module):
    ...
    # def compute(self, anchor, sample) -> torch.FloatTensor:
    ...
    def compute(self, anchor, sample):
        anchor = F.normalize(anchor, dim=-1, p=2)
        sample = F.normalize(sample, dim=-1, p=2)
        similarity = torch.sum(anchor * sample, dim=1)
        
        loss = torch.sum(similarity, dim=0)
        return loss.mean()
    
    def forward(self, h_pred, h_target):
        # pos_mask =  torch.eye(h_pred.size(0), dtype=torch.float32, device=h_pred.device)
        l = self.compute(anchor=h_pred, sample=h_target)
        return l
