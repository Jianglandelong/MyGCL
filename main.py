...
def train(encoder_model, contrast_model, features, edges, optimizer, alpha, beta, gamma, args):
    encoder_model.train()
    optimizer.zero_grad()

    _, _, h1_lp_pred, h1_hp_pred, h2_lp_target, h2_hp_target = encoder_model(features, edges)
    if args.lp_off == 1:
        l1 = torch.tensor(0.0).to(h1_lp_pred.device)
    else:
        l1 = contrast_model(h_pred = h1_lp_pred, h_target = h2_lp_target.detach())
    if args.hp_off == 1:
        l2 = torch.tensor(0.0).to(h1_hp_pred.device)
    else:
        l2 = contrast_model(h_pred = h1_hp_pred, h_target = h2_hp_target.detach())
    cross_view_loss = l1 * alpha + l2 * beta
    if args.lp_off == 0 and args.hp_off == 0:
        if gamma != 0:
            cross_pass_loss = contrast_model(h_pred = h1_lp_pred, h_target = h1_hp_pred) * gamma
        elif args.delta_1 != 0:
            cross_pass_loss = contrast_model(h_pred = h1_lp_pred, h_target = h2_hp_target) * args.delta_1
        elif args.delta_2 != 0:
            cross_pass_loss = contrast_model(h_pred = h1_hp_pred, h_target = h2_lp_target) * args.delta_2
        else:
            cross_pass_loss = torch.tensor(0.0).to(h1_lp_pred.device)
...
def main(args):
    ...
    for trial in range(args.ntrials):
        ...
        for epoch in range(1, args.epochs + 1):
            loss = train(encoder_model, contrast_model, features, edges, optimizer, args.alpha, args.beta, args.gamma, args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-delta_1', type=float, default=0.0, help='Hlp to Zhp')
    parser.add_argument('-delta_2', type=float, default=0.0, help='Hhp to Zlp')
    ...
    parser.add_argument('-lp_off', type=int, default=0)
    parser.add_argument('-hp_off', type=int, default=0)
